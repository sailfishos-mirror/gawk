...
nextfile(IOBUF **curfile, bool skipping)
{
	...
	for (; i < get_number_si(ARGC_node->var_value);  i++) {
		...
		if (! do_traditional) {
			...
			ARGIND_node->var_value = make_number((double) i);
		}
...
do_close(int nargs)
{
	...
	if (rp == NULL) {	/* no match, return -1 */
		...
		DEREF(tmp);
		return make_number((double) -1.0);
	}
	DEREF(tmp);
	fflush(stdout);	/* synchronize regular output */
	tmp = make_number((double) close_redir(rp, false, how));
	rp = NULL;
	/*
...
do_getline_redir(int into_variable, enum redirval redirtype)
{
	...
	if (errcode != 0) {
		if (errcode != -1)
			update_ERRNO_int(errcode);
		return make_number((double) retval);
	}
